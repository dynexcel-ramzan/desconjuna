        ...
        for agent in list:
            ...
            for agent_invoice in self:
                if  agent_invoice.agent_id.id ==  agent.id  and agent_invoice.commission_settled == False:
                    ...
                    invoices = str(agent_invoice.invoice_date) +'- ' + str(agent_invoice.name) +' '+ str(agent_invoice.ref)
                    for inv_line in agent_invoice.invoice_line_ids:
                        if inv_line.package_id.shipping_grade == 'fresh':
                            sum_qty += inv_line.quantity
                            sum_coms_rate += inv_line.commission_rate
                    invoice_line_vals.append((0, 0, {
                        'name': invoices,
                        'account_id': 19,
                        'quantity': sum_qty,
                        'price_unit': agent_invoice.partner_id.comission,
                        'analytic_account_id': 93,
                    }))
                        #total_commission_amount = total_commission_amount + agent_invoice.commission
                        # invoices = invoices + ', ' +  str(agent_invoice.date_invoice) +'- ' + str(agent_invoice.number) +' '+ str(agent_invoice.name)
